...
class Generator(nn.Module):
    ...
    def __init__(self, input_dims, output_dims):
        ...
        self.fc1 = nn.Sequential(nn.Linear(input_dims,256), nn.LeakyReLU(0.02))
        self.fc2 = nn.Sequential(nn.Linear(256,512), nn.LeakyReLU(0.02))
        self.fc3 = nn.Sequential(nn.Linear(512,1024), nn.LeakyReLU(0.02))
        # output hidden layer
...
def show_result(G_net, z_, num_epoch, show=False, save=False, path='result.png'):
    ...
    epoch_steps = [10,20,50,100]
    
    generated_images = G_net(z_)
    plt.figure(figsize=(10, 10))

    if num_epoch in epoch_steps:
        for i in range(len(generated_images)):
            ...
            #Plot the figures
            plt.subplot(5, 5, i+1)
            plt.suptitle("Images generated at epoch :- "+str(num_epoch))
